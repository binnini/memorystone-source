...
턴 진행을 **동기 함수** 방식으로 구현했습니다. `EndAction()`이나 `ResolveMonsterAction()`을 한 번 부르면 그 안에서 해당 페이즈의 규칙과 턴 경계 16단계가 끝까지 실행되고, 호출이 돌아온 시점에 전투 상태는 이미 다음 상태입니다. 순서는 함수 안의 줄 순서 한 곳에만 적혀 있습니다.

대안은 **액션 큐** 방식입니다(Slay the Spire 계열). "5 피해", "한 칸 이동", "턴 시작 유물 발동" 같은 규칙 하나하나를 액션 객체로 큐에 넣고, 매니저가 프레임에 걸쳐 하나씩 실행합니다. 유물이나 상태이상은 사건을 구독해 큐 중간에 자기 액션을 끼워 넣고, 연출은 각 액션이 스스로 품습니다.

| | 동기 함수 (현재) | 액션 큐 |
|---|---|---|
| 턴 순서가 적힌 곳 | 함수의 줄 순서 한 곳 | 큐에 액션을 끼워 넣는 모든 코드의 삽입 위치 |
| 씬 없는 테스트 | 함수 호출 한 번 = 결과. 그대로 검증 | 즉시 실행 모드가 따로 필요하고, 실제 모드와 같다는 보장을 별도로 세워야 함 |
| 몬스터 예고 = 실제 피해 | 예고와 실행이 같은 함수를 부르므로 일치 | 큐에 끼어드는 반응이 결과를 바꿀 수 있어 예고 시점에 큐를 가상 실행해야 함 |
| 세이브 | 상태 스냅샷 하나 | 큐 실행 도중 저장하려면 큐 내용까지 직렬화 |
| 시드 재현 | 난수 소비 순서가 함수 순서에 고정 | 소비 순서가 큐 삽입 순서에 묶여 순서 버그가 재현성 버그가 됨 |
| 다음 턴에 발동하는 효과 | 예약 슬롯(`PendingEffects`)을 두고 경계에서 꺼냄 | 발동 시점이 붙은 액션 하나. 슬롯 불필요 |
| 유물 · 상태이상의 반응 | 규칙 코드의 해당 자리에 분기가 들어감 | 구독 모델. 규칙 코드를 건드리지 않음 |
| 카드 사용 중 플레이어 선택 | 선택을 먼저 받아 인자로 넘겨야 함 | 큐를 멈추고 입력 대기 가능 |

동기 방식은 테스트 · 예고 일치 · 세이브 · 재현성이 강하고, 지연 효과와 반응형 유물의 표현력이 약합니다. 액션 큐는 그 반대입니다. 이 게임은 몬스터 예고가 핵심 약속이고 시드 재현과 씬 없는 테스트에 크게 기대고 있어 동기 방식을 택했습니다. 표현력의 약점은 `PendingEffects` 같은 예약 슬롯과 턴 경계의 고정 순서로 흡수했습니다.
