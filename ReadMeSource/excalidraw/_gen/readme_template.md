...
첫째, 정보 은닉을 UI 코드의 조심성이 아니라 자료구조에서 처리했습니다. Unity 쪽은 `GetSafeCellInfo`가 돌려주는 구조체만 받고, 보이지 않는 칸의 이벤트나 랜드마크는 그 구조체에 아예 담기지 않습니다. UI가 실수로 모르는 칸의 정보를 그리는 버그는 그 정보가 도착하지 않으므로 생길 수 없습니다. 화면의 어둠도 같은 구조체에서 마스크로 만들어지므로 규칙상의 시야와 화면의 시야가 항상 일치합니다.

둘째, 시야 단계를 쓰는 경로를 둘로 나눠 좁혔습니다. `SetVisibility`는 단계를 올리기만 하는 public 함수라서, 턴 경계 · 정찰 카드 · 횃불 · 이벤트가 어떤 순서로 몇 번을 부르든 결과는 가장 높은 단계로 수렴합니다. 호출하는 쪽은 현재 단계를 확인할 필요가 없습니다. 단계를 내리는 `ForceVisibility`는 private이고, 시야에서 벗어난 칸을 `Hinted`로 내릴 때와 세이브를 복원할 때만 이 클래스 안에서 씁니다. 「한 번 본 칸은 다시 모르게 되지 않는다」는 규칙이 접근 제한자로 보증되고, 시야가 낮아지는 경로가 코드 몇 줄로 한정됩니다. 두 함수만이 `states`를 쓰므로 실제로 바뀔 때만 올라가는 `Version` 카운터가 정확해지고, 렌더 쪽은 정수 하나를 비교해 다시 그릴지 결정합니다.
