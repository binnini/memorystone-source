...
플레이어가 휘둘러치기를 써서 몬스터 A와 B를 때립니다. A는 죽고, B는 한 칸 밀려납니다.

1. 카드를 클릭하면 `MapCombatController`가 `CombatState`에 연산을 요청합니다.
2. `CombatState`가 그 자리에서 연산을 끝냅니다. A 사망, B 피해와 이동. 결과가 전투 기록에 순서대로 남습니다. 화면은 아직 그대로입니다.
3. 컨트롤러가 전투 기록을 읽어 갑니다.
4. 전투 기록과 화면의 전후 스냅샷을 `CombatTimelineAssembler`에 넘겨 연출 순서를 만듭니다. "공격 모션 → 타격 → A 사망 → B 넉백" 순입니다.
5. 완성된 타임라인을 `PresentationScheduler`에 넘깁니다. 여기서부터 시간이 흐릅니다.
6. 스케줄러가 한 항목씩 화면 지시를 내립니다.
7. 지시를 컨트롤러가 실행합니다. 애니메이션, 이펙트, 캐릭터 이동이 이때 일어납니다.

전투 연산은 2에서 끝납니다. 6~7이 재생되는 동안에도 결과는 이미 정해져 있습니다.
